...
1. 프롬프트에서 다음 정보를 제공합니다.
   ...
   * **프로젝트 첫 번째 함수의 템플릿 선택**: **Azure Event Grid 트리거**를 선택합니다.
...
1. 함수용 멤버 변수를 추가하려면 `// INSERT member variables below here` 주석을 찾은 다음 그 아래에 다음 코드를 삽입합니다.
...
1. **Run** 메서드 선언을 추가하려면 `// INSERT Run method below here` 주석을 찾은 다음 그 아래에 다음 코드를 삽입합니다.
...
1. Event Grid 이벤트 데이터 관련 정보 데이터를 로깅하려면 `// INSERT log statement below here` 주석을 찾은 다음 그 아래에 다음 코드를 삽입합니다.
...
1. **ADT_SERVICE_URL** 환경 변수가 정의되었는지 확인하려면 `// INSERT environment variable check below here` 주석을 찾은 다음 그 아래에 다음 코드를 삽입합니다.
...
1. 모든 예외가 로깅되도록 하려면 `// INSERT try/catch block below here` 주석을 찾은 다음 그 아래에 다음 코드를 삽입합니다.
...
1. 함수 앱 서비스 주체를 사용하여 ADT에 인증한 다음 클라이언트 인스턴스를 만들려면 `// INSERT authentication code below here` 주석을 찾은 다음 그 아래에 다음 코드를 삽입합니다.
...
1. Event Grid 이벤트 처리를 시작하려면 `// INSERT event processing code below here` 주석을 찾은 다음 그 아래에 다음 코드를 삽입합니다.
...
1. ADT 트윈을 업데이트하는 코드를 추가하려면 `// INSERT ADT update code below here` 주석을 찾은 다음 그 아래에 다음 코드를 삽입합니다.
...
1. URL 아래에서 **Params** 탭을 선택하고 다음 정보를 입력합니다.
...
1. 프롬프트에서 다음 정보를 제공합니다.
   ...
   * **템플릿 필터 선택**: **모두**를 선택합니다.
   * **함수의 템플릿 선택**: **Azure Event Grid 트리거**를 선택합니다.
...
1. 원본 Cheese Cave Device의 부모 Cheese Cave를 찾으려면 `// INSERT Find the device parent model (the Cheese Cave)`  주석을 찾은 다음 그 아래에 다음 코드를 삽입합니다.
...
1. 부모 Cheese Cave를 업데이트하려면 `// INSERT Update the parent` 주석을 찾은 다음 그 아래에 다음 코드를 삽입합니다.
...
1. 이벤트에 Cheese Cave Device 원격 분석이 포함되어 있는지를 확인하는 코드를 추가하려면 `// INSERT check telemetry below here` 주석을 찾은 다음 그 아래에 다음 코드를 삽입합니다.
...
1. 이벤트를 처리하고 TSI용 메시지를 만드는 코드를 추가하려면 `// INSERT TSI Event creation below here` 주석을 찾은 다음 그 아래에 다음 코드를 삽입합니다.
