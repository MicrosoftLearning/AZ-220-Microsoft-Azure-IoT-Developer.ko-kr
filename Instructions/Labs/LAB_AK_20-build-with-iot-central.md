...
> **참고**:
>  이 작업을 건너뛰고 모든 코드를 앱에 로드하려면 Program.cs의 모든 내용을 다운로드하여 프로젝트의 Program.cs 파일에 복사합니다. 이 코드를 복사하고 연결 및 구독 문자열을 바꾸면 다음 작업으로 바로 이동하여 테스트를 시작합니다. 이 과정의 랩 3 "개발 환경 설정"에서 ZIP 파일을 다운로드하고 내용을 로컬로 추출하여 랩 리소스를 포함하는 GitHub 리포지토리를 복제했습니다. 추출된 폴더 구조에는 다음 폴더 경로가 포함됩니다.
